...
```{r}
# Import data set
data <- read.csv("D:/School/Walmart_Store_sales.csv", header = TRUE) 
head(data)
...
n <- nrow(result_bs)
numeric(n)
result1 <- data.frame(result_bs)
for (var_name in names(result_bs)) {
  # Get the variable
  var <- result_bs[[var_name]]
  ...
  print(paste("Jackknife estimate of bias for", var_name, "is", format(bias_jk, digits = 10)))
  
  variance_jk <- (n - 1) * mean((theta_jacks - mean(theta_jacks))^2)
  print(paste("Jackknife estimate of variance for", var_name, "is", format(variance_jk, digits = 10)))
}
```
